...
namespace ActionTools
{
	...
	QString WindowHandle::title() const
	{
        ...
        if(XFetchName(PlatformInfo::display(), mValue, &str))
	...
	QString WindowHandle::classname() const
	{
        ...
        if(XGetClassHint(PlatformInfo::display(), mValue, hint))
	...
	QRect WindowHandle::rect(bool useBorders) const
	{
        ...
        if(!XGetWindowAttributes(PlatformInfo::display(), mValue, &windowAttributes))
			return QRect();

		Window unused;
		int positionX, positionY;

        XTranslateCoordinates(PlatformInfo::display(), mValue, windowAttributes.root, -windowAttributes.border_width, -windowAttributes.border_width,
	...
	int WindowHandle::processId() const
	{
		...
		if(atomPid == None)
            atomPid = XInternAtom(PlatformInfo::display(), "_NET_WM_PID", True);
        ...
        if(XGetWindowProperty(PlatformInfo::display(), mValue, atomPid, 0, 1, False, XA_CARDINAL,
	...
	{
#ifdef Q_OS_UNIX
        return XDestroyWindow(PlatformInfo::display(), mValue);
#endif
#ifdef Q_WS_WIN
	...
	{
#ifdef Q_OS_UNIX
        return XKillClient(PlatformInfo::display(), mValue);
#endif
#ifdef Q_WS_WIN
		int id = processId();
	...
	bool WindowHandle::setForeground() const
	{
		...
		if(atomActiveWindow == None)
            atomActiveWindow = XInternAtom(PlatformInfo::display(), "_NET_ACTIVE_WINDOW", False);
        ...
        event.xclient.display = PlatformInfo::display();
        ...
        return XSendEvent(PlatformInfo::display(), windowAttributes.screen->root, False, SubstructureNotifyMask | SubstructureRedirectMask, &event);
#endif
#ifdef Q_WS_WIN
		if(IsIconic(mValue))
	...
	{
#ifdef Q_OS_UNIX
        return XIconifyWindow(PlatformInfo::display(), mValue, DefaultScreen(PlatformInfo::display()));
#endif
#ifdef Q_WS_WIN
	...
	bool WindowHandle::maximize() const
	{
		...
		if(atomState == None)
            atomState = XInternAtom(PlatformInfo::display(), "_NET_WM_STATE", False);
		static Atom atomMaximizeVert = None;
		if(atomMaximizeVert == None)
            atomMaximizeVert = XInternAtom(PlatformInfo::display(), "_NET_WM_STATE_MAXIMIZED_VERT", False);
		static Atom atomMaximizeHorz = None;
		if(atomMaximizeHorz == None)
            atomMaximizeHorz = XInternAtom(PlatformInfo::display(), "_NET_WM_STATE_MAXIMIZED_HORZ", False);
	...
	{
#ifdef Q_OS_UNIX
        return XMoveWindow(PlatformInfo::display(), mValue, position.x(), position.y());
#endif
#ifdef Q_WS_WIN
	...
	bool WindowHandle::resize(QSize size, bool useBorders) const
	{
        ...
        return XResizeWindow(PlatformInfo::display(), mValue, size.width(), size.height());
#endif
#ifdef Q_WS_WIN
		if(!useBorders)
		{
	...
	WindowHandle WindowHandle::foregroundWindow()
	{
        ...
        XGetInputFocus(PlatformInfo::display(), &focus, &revert);
	...
	QList<WindowHandle> WindowHandle::windowList()
	{
		...
		if(!net_clients)
            net_clients = XInternAtom(PlatformInfo::display(), "_NET_CLIENT_LIST_STACKING", True);
        ...
        XGetWindowProperty(PlatformInfo::display(), PlatformInfo::appRootWindow(), net_clients, 0, 256 * sizeof(Window), False, AnyPropertyType,
